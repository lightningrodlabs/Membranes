...
///
pub fn publish_vouchTreshold(
   required_count: usize,
   by_role: &str,
   for_role: &str,
) -> ExternResult<EntryHash> {
   let vouch_th = VouchThreshold {
      required_count,
      by_role: by_role.to_string(),
...
///
pub fn publish_createEntryCountThreshold(
...
pub fn init_membranes(_: ()) -> ExternResult<()> {
   ...
   let createItemThresholdEh: EntryHash = publish_createEntryCountThreshold(
      MyAppEntryType::from(TaskerEntryTypes::TaskItem.try_into().unwrap()),
      1,
   )?;
   let createManyItemThresholdEh: EntryHash = publish_createEntryCountThreshold(
      MyAppEntryType::from(TaskerEntryTypes::TaskItem.try_into().unwrap()),
      3,
   )?;

   /// Membranes
   let progenitorMembraneEh: EntryHash = call_membranes_zome(
      "publish_membrane",
      Membrane {
         threshold_ehs: vec![progenitor_th_eh.clone()],
      },
   )?;
   let createItemsMembraneEh: EntryHash = call_membranes_zome(
      "publish_membrane",
      Membrane {
         threshold_ehs: vec![createItemThresholdEh.clone()],
      },
   )?;
   let participantVouchMembraneEh: EntryHash = call_membranes_zome(
      "publish_membrane",
      Membrane {
         threshold_ehs: vec![participantVouchThresholdEh],
      },
   )?;
   ...
   let complexMembraneEh: EntryHash = call_membranes_zome(
      "publish_membrane",
      Membrane {
         threshold_ehs: vec![createManyItemThresholdEh, steererVouchThresholdEh],
      },
   )?;
   ...
   let _progenitorRoleEh: EntryHash = call_membranes_zome(
      ...
      MembraneRole {
         ...
         privileges: vec![/* FIXME */],
         entering_membrane_ehs: vec![progenitorMembraneEh.clone()],
      },
   )?;
   let participantRoleEh: EntryHash = call_membranes_zome(
      ...
      MembraneRole {
         ...
         privileges: vec![/* FIXME */],
         entering_membrane_ehs: vec![createItemsMembraneEh.clone()],
      },
   )?;
   ...
   let _editorRoleEh: EntryHash = call_membranes_zome(
      ...
      MembraneRole {
         ...
         privileges: vec![/* FIXME */],
         entering_membrane_ehs: vec![participantVouchMembraneEh.clone()],
      },
   )?;
   let _steererRoleEh: EntryHash = call_membranes_zome(
      "publish_role",
      MembraneRole {
         name: "steerer".to_string(),
         privileges: vec![/* FIXME */],
         entering_membrane_ehs: vec![complexMembraneEh.clone()],
      },
   )?;
   ...
   let maybe_proof: Option<SignedActionHashed> = call_membranes_zome(
      ...
      HasRoleInput {
         ...
         role_eh: participantRoleEh.into(),
      },
   )?;
