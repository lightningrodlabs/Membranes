...
pub fn get_task_item(eh: EntryHash) -> ExternResult<Option<(TaskItem, bool)>> {
   ...
   /// Get TaskItem
   let Ok(task_item) = zome_utils::get_typed_from_eh::<TaskItem>(eh.clone().into())
   // FIXME should get latest and not content
   else {
      return Ok(None);
   };
...
pub fn get_list_items(list_eh: EntryHash) -> ExternResult<Vec<(EntryHash, TaskItem, bool)>> {
   ...
   for link in item_links.into_iter() {
      let item_eh: EntryHash = EntryHash::try_from(link.target).unwrap();
      let Some((item, is_complete)) = get_task_item(item_eh.clone())? else {
         continue;
      };
...
pub fn get_all_lists(_: ()) -> ExternResult<Vec<(EntryHash, TaskList)>> {
   ...
   let list_pairs: Vec<(EntryHash, TaskList)> = link_pairs
      .into_iter()
      .map(|(list, link)| {
         let list_eh: EntryHash = EntryHash::try_from(link.target).unwrap();
         (list_eh, list)
      })
      .collect();
   /// Done
