...
use hdk::prelude::*;
use membranes_types::{
   ClaimRoleInput, HasRoleInput, Membrane, MembraneInput, MembraneRole, RoleClaim,
};
use tasker_model::*;
...
pub fn am_i_editor() -> ExternResult<Option<(EntryHash, RoleClaim)>> {
   ...
   let Some(pair): Option<(EntryHash, MembraneRole)> =
      call_membranes_zome("get_role_with_name", "editor".to_string())?
   else {
      warn!("'editor' role not found");
      return zome_error!("'editor' role not found");
   };
   let maybe_signed_role_claim: Option<SignedActionHashed> = call_membranes_zome(
      "has_role",
      HasRoleInput {
         subject: me,
         role_eh: pair.0,
   ...
   debug!(
      "am_i_editor() maybe_signed_role_claim: {:?}",
      maybe_signed_role_claim
   );
