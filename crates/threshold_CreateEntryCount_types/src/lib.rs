...
impl CreateEntryCountThreshold {
   ...
   pub fn verify(
      ...
      for signed_ah in signed_ahs {
         let Ok(record) = must_get_valid_record(signed_ah.ah) else {
            return Ok(false);
         };
         let action = record.action().clone();
         let valid_signature =
            verify_signature(subject.clone(), signed_ah.signature, action.clone())?;
         if !valid_signature {
            continue;
         }
         ...
         let EntryType::App(app_entry_type) = create.entry_type.clone() else {
            continue;
         };
