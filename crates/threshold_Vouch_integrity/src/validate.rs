   ...
   match op {
      ...
      Op::StoreEntry(storeEntry) => {
         ...
         return validate_entry(
            ...
            storeEntry.entry,
            Some(creation_action.entry_type()),
         );
      },
...
pub fn validate_entry(
   ...
   let result = match entry.clone() {
      ...
      Entry::App(_entry_bytes) => {
         ...
         let entry_def_index =
            validate_app_entry(creation_action, app_entry_def.entry_index(), entry);
         entry_def_index
      },
   };
   /// Done
...
fn validate_proof(
   author: AgentPubKey,
   proof: ThresholdReachedProof,
) -> ExternResult<ValidateCallbackResult> {
   let threshold_entry = must_get_entry(proof.threshold_eh.clone().into())?
      .as_content()
      .to_owned();
   let threshold = MembraneThreshold::try_from(threshold_entry)?;
   if threshold.type_name != VOUCH_THRESHOLD_NAME {
      return Ok(ValidateCallbackResult::Invalid(format!(
         "Threshold not a VouchThreshold"
      )));
   }
   ...
   Ok(if pass {
      ...
      ValidateCallbackResult::Invalid(format!(
         "Failed validating proof for \"{}\" threshold.",
...
///
pub fn verify_vouch_threshold(
   vt: &VouchThreshold,
   subject: AgentPubKey,
   signed_ahs: Vec<SignedActionHash>,
) -> ExternResult<bool> {
   let vouch_entry_index =
      get_variant_index::<VouchThresholdEntry>(VouchThresholdEntryTypes::Vouch)?;
   ...
   for signed_ah in signed_ahs {
      let Ok(record) = must_get_valid_record(signed_ah.ah.clone()) else {
         return Ok(false);
      };
      let action = record.action().clone();
      /// Must find enough Vouch CreateEntry actions with the correct "for_role" by authors who have the "by_role" role
      let Action::Create(create) = action.clone() else {
         continue;
      };
      let EntryType::App(app_entry_def) = create.entry_type.clone() else {
         continue;
      };
   ...
   vouch_map = vouch_map
      ...
      .collect();
   debug!(
      "verify_vouch_threshold() confirmed vouches = {}",
      vouch_map.len()
   );
