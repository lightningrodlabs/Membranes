...
fn validate_role_claim(role_claim: RoleClaim) -> ExternResult<ValidateCallbackResult> {
   ...
   ///
   let membrane_claim_entry = must_get_entry(role_claim.membrane_claim_eh.clone().into())?
      .as_content()
      .to_owned();
   let membrane_claim = MembraneCrossedClaim::try_from(membrane_claim_entry)?;
   let role_entry = must_get_entry(role_claim.role_eh.clone().into())?
      .as_content()
      .to_owned();
...
///
fn validate_membrane_claim(
   membrane_claim: MembraneCrossedClaim,
) -> ExternResult<ValidateCallbackResult> {
   let membrane_entry = must_get_entry(membrane_claim.membrane_eh.clone().into())?
      .as_content()
      .to_owned();
   let membrane = Membrane::try_from(membrane_entry)?;
   let mut i = 0;
   for threshold_eh in membrane.threshold_ehs.iter() {
      let threshold_entry = must_get_entry(threshold_eh.clone().into())?
         .as_content()
         .to_owned();
      ...
      let Ok(proof_record) = must_get_valid_record(proof_ah) else {
         return Ok(ValidateCallbackResult::Invalid(format!(
            "Threshold proof record not found ({})",
            i
         )));
      };
      let RecordEntry::Present(entry) = proof_record.entry else {
         return Ok(ValidateCallbackResult::Invalid(format!(
            "Threshold proof entry not present ({})",
            i
         )));
      };
      ...
      if proof.threshold_eh != hash_entry(threshold.clone())? {
         return Ok(ValidateCallbackResult::Invalid(format!(
            "Threshold proof EntryHash mismatch ({})",
            i
         )));
      }
      i += 1;
