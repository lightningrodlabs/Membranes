...
fn validate_proof(
   ...
   if threshold.type_name != CREATE_ENTRY_COUNT_THRESHOLD_NAME {
      return Ok(ValidateCallbackResult::Invalid(format!(
         "Threshold not of type CreateEntryCountThreshold"
      )));
   }
   let Ok(cec) = CreateEntryCountThreshold::try_from(threshold.data) else {
      return Ok(ValidateCallbackResult::Invalid(format!(
         "Threshold not a CreateEntryCountThreshold"
      )));
   };
   ...
   Ok(if pass {
      ...
      ValidateCallbackResult::Invalid(format!(
         "Validation failed for proving \"{}\" threshold.",
