...
pub fn get_all_thresholds_Vouch(_: ()) -> ExternResult<Vec<VouchThreshold>> {
   let thresholds: Vec<MembraneThreshold> = call_self_cell(
      "zMembranes",
      "get_all_thresholds",
      Some(VOUCH_THRESHOLD_NAME),
   )?;
   debug!(
      "get_all_thresholds_Vouch() thresholds.len = {}",
      thresholds.len()
   );
...
fn claim_threshold_Vouch(input: ClaimThresholdInput) -> ExternResult<Option<ActionHash>> {
   if input.threshold.type_name != VOUCH_THRESHOLD_NAME {
      return zome_error!(
         ...
         VOUCH_THRESHOLD_NAME,
         input.threshold.type_name
      );
   }
   let th: VouchThreshold =
      VouchThreshold::try_from(input.threshold.data.clone()).expect("Corrupt threshold data");
   /// Get Threshold's by role entry
   let maybe_th_by_role: Option<MembraneRole> =
      call_self_cell("zMembranes", "get_role_by_name", th.by_role.clone())?;
   if maybe_th_by_role.is_none() {
      let msg = format!(
         "Could not get Role declared in VouchThreshold.by_role: {}",
         th.by_role
      );
   ...
   let link_pairs = zome_utils::get_typed_from_links::<Vouch>(zome_utils::link_input(
      input.subject.clone(),
   ...
   for (vouch, link) in link_pairs {
      ...
      if vouch.subject != input.subject {
         continue;
      }
      /// Vouch must be for the right role
      if vouch.for_role != th.for_role {
         continue;
      }
      /// Get vouch's author
   ...
   debug!(
      "claim_threshold_Vouch() author_map.len 1 = {:?}",
      author_map.len()
   );
   ...
   author_map = author_map
      .into_iter()
      .filter(|(author, (_vouch, link))| {
         let subject: AgentPubKey = author.to_owned().into();
         let payload = HasRoleInput {
            subject,
            role_eh: by_role_eh.clone(),
         };
         let maybe_role_claim_sah: ExternResult<Option<SignedActionHashed>> =
            call_self_cell("zMembranes", "has_role", payload);
         if maybe_role_claim_sah.is_err() || maybe_role_claim_sah.as_ref().unwrap().is_none() {
            return false;
         }
         let role_claim_sah = maybe_role_claim_sah.unwrap().unwrap();
         /// Get Vouch's SignedActionHashed
         let target: AnyDhtHash = link.target.clone().into_entry_hash().unwrap().into();
         let maybe_vouch_sah = get(target, GetOptions::network());
         if maybe_vouch_sah.is_err() || maybe_vouch_sah.as_ref().unwrap().is_none() {
            let msg = format!(
               "Could not get VouchReceived link's target entry {}",
               link.target
            );
            warn!(msg);
            return false;
         ...
         signed_ahs.push(SignedActionHash {
            signature: vouch_sah.signature.clone(),
            ah: vouch_sah.action_address().to_owned(),
         });
         signed_ahs.push(SignedActionHash {
            signature: role_claim_sah.signature.clone(),
            ah: role_claim_sah.action_address().to_owned(),
         });
         return true;
      })
      .collect();
   debug!(
      "claim_threshold_Vouch() author_map.len 2 = {:?}",
      author_map.len()
   );
